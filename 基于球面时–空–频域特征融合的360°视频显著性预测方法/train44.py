...
def train(train_data, val_data, model, device, criterion, lr=0.001, EPOCHS=10, model_name='Model'):  # 默认参数没有传递main
    ...
    writer = SummaryWriter(
        os.path.join(config.runs_data_dir, model_name + '_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + '/'))
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in train_data:
            ...
            if counter_train % 20 == 0:  # epoch当前训练轮次   counter_train当前训练批次batch的计数器  len训练数据集的批次数，最后一个平均损失
                print("Epoch {}......Step: {}/{}....... Average Loss for Epoch: {}".format(epoch, counter_train,
                                                                                           len(train_data),
        ...
        if epoch % 10 == 0:
            ...
            ckp_path = os.path.join(config.ckp_dir,
                                    model_name + '_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
            os.mkdir(ckp_path)
            torch.save({
                'epoch': epoch,
...
if __name__ == '__main__':
    ...
    train_video360_dataset = RGB_and_OF(config.frames_dir, config.optical_flow_dir, config.gt_dir, config.frequency_dir,
                                        video_names_train, config.sequence_length, split='train',
                                        resolution=config.resolution)
    val_video360_dataset = RGB_and_OF(config.frames_dir, config.optical_flow_dir, config.gt_dir, config.frequency_dir,
                                      video_names_train, config.sequence_length, split='validation',
                                      resolution=config.resolution)
    ...
    print(model)
    model = train(train_data, val_data, model, device, criterion, lr=config.lr, EPOCHS=config.epochs,
